...
If you encounter the following errors when running the development server:
- **Loading chunk failed** errors (e.g., `Loading chunk component---cache-dev-404-page-js failed`)
- **Page hangs on load** at localhost:8000
- **Multiple unhandled runtime errors** in commons.js or dev-loader.js
- **Failed to fetch** errors

These are symptoms of a corrupted Gatsby cache. Follow these steps to fix:
...
1. **Close all terminals and browser tabs** related to the project
...
3. **Manually delete cache directories**:
   ```powershell
...
4. **Clear npm cache** (optional but can help):
   ```bash
...
3. Ensure no antivirus software is interfering with file operations

### Common Causes

The Gatsby cache can become corrupted when:
- The development bundle gets interrupted during compilation
- File watchers lose sync with actual files
- Hot module replacement gets into a bad state
- Multiple development servers run simultaneously
- System runs out of memory during build

### Prevention Tips

- Always stop the dev server cleanly with `Ctrl+C`
- Run `npm run clean` periodically during development
- Don't run multiple instances of `npm run develop`
- Keep your dependencies up to date

### Chat Widget Loading Issues

If you see a stuck loading message with:
- **"How can we help you today? Type message here..."**
- **"Preparing requested page"**
- Chat widget stuck in corner of the page

This is the Chatlio chat widget failing to load properly. Solutions:

#### Quick Fix

1. **Disable Chatlio temporarily** by commenting it out in `src/html.js`:
   ```js
   {/* Temporarily disable Chatlio
...
Add a condition to only load Chatlio in production:
...
1. **Update Chatlio widget ID** if it's expired or invalid
2. **Remove Chatlio entirely** if it's no longer needed
3. **Add error handling** to prevent it from blocking page load:
   ```js
   try {
     // Chatlio initialization code
   } catch (e) {
     console.warn('Chatlio failed to load:', e);
   }
   ```

### Common Widget Issues

Third-party widgets can cause loading issues when:
- API keys or widget IDs are invalid/expired
- Network requests to widget servers fail
- Widget scripts have errors or incompatibilities
- Development environment blocks external scripts
