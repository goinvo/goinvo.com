              ...
              <ul className="list--unstyled carousel__menu-items">
                {
                  this.props.menuItems.map((item, i) => {
                    return (
                      <li key={item} className={`carousel__menu-item ${i === this.state.activeIndex ? 'carousel__menu-item--selected' : ''}`}>
                        <button className="button button--link" onClick={() => this.goTo(i)}>{item}</button>
                        <span className="carousel__selected-indicator"></span>
                      </li>
                    )
                  })
                }
